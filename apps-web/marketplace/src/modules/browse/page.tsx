...
const BrowsePage: CustomNextPage = () => {
  ...
  const { value: grades, handler: gradesHandler } = useQueryParamArray<Grade>('grades')
  const { value: basePlanTypes, handler: basePlanTypesHandler } =
    useQueryParamArray<BasePlanType>('basePlanTypesType')
  const { value: subjectIds, handler: subjectIdsHandler } = useQueryParamArray('subjectIds')
  const { value: tutorIds, handler: tutorIdsHandler } = useQueryParamArray('tutorIds')
  ...
  const where = useMemo(() => {
    const shouldAddFusionType =
      liveCourseType.includes(LiveCourseType.LIVE) || liveCourseType.includes(LiveCourseType.ONSITE)
    ...
    return {
      name: name || undefined,
      liveCourseType: computedLiveCourseType.length > 0 ? computedLiveCourseType : undefined,
  ...
  const { data, isFetching } = reactQueryClient.liveCourse.getLiveCourses.useQuery(
    { query: { where, orderBy } },
      ...
      <div className="container flex flex-col md:flex-row md:gap-6 py-6 md:py-10 flex-1">
        <section className="sticky top-[calc(24px+var(--navbar))] md:top-[calc(40px+var(--navbar))] hidden md:w-[314px] md:flex flex-col gap-4 self-start max-h-[calc(100vh-var(--navbar)-80px)] overflow-y-auto hide-scrollbar">
          <div className="flex items-center">
            <FontAwesomeIcon icon={faSort} className="w-3 h-3 md:w-4 md:h-4 mr-1" />
            <label className="mr-2 whitespace-nowrap">เรียงตาม</label>
            <Select value={select} onValueChange={(value) => setSelect(value as BrowseSort)}>
              ...
              <SelectContent>
                ...
                <SelectItem value={BrowseSort.START_DESC}>วันเริ่มคอร์ส ก่อน-หลัง</SelectItem>
                <SelectItem value={BrowseSort.START_ASC}>วันเริ่มคอร์ส หลัง-ก่อน</SelectItem>
                <SelectItem value={BrowseSort.PRICE_DESC}>ราคา มาก-น้อย</SelectItem>
                <SelectItem value={BrowseSort.PRICE_ASC}>ราคา น้อย-มาก</SelectItem>
              </SelectContent>
            </Select>
          </div>
            ...
            <div className="flex gap-2">
              ...
              <Dialog>
                <DialogTrigger asChild>
                  <Button
                    variant="outline"
                    leftIcon={<FontAwesomeIcon icon={faBarsFilter} className="w-4 h-4" />}
                  >
                    ตัวกรอง
                  </Button>
                </DialogTrigger>
                <DialogContent fullPage>
                  <DialogHeader>
                    <DialogTitle>
                      <div className="flex items-center justify-between">
                        <div className="flex items-center">
                          <FontAwesomeIcon icon={faBarsFilter} className="w-4 h-4 mr-1" />
                          <label className="whitespace-nowrap">ตัวกรอง</label>
                        </div>
                  ...
                  <section className="container flex flex-col gap-4">{FilterSection}</section>
                  <DialogFooter>
                    <DialogClose asChild>
                      <Button
                        fullWidth
                        variant="outline"
                        leftIcon={<FontAwesomeIcon icon={faSearch} className="h-4 w-4" />}
                      >
                        แสดงผลลัพธ์
                      </Button>
                    </DialogClose>
  ...
  return (
    <div className="mt-6 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4 sm:gap-6">
      {Array.from({ length: 7 }).map((_, index) => (
        <div className="aspect-[3/4] bg-accent rounded-lg animate-pulse" key={index} />
      ))}
...
const EmptyState = () => {
  return (
    <div className="flex flex-col flex-1 items-center mt-48">
      <NextImage src={MonkeyCry} width={123} height={112} alt="monkey cry" className="mb-2" />
