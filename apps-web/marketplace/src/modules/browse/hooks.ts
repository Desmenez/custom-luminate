...
export function useQueryParamArray<TValue extends string = string>(
  key: string
) {
  const router = useRouter();
  const { query } = router;
...
export type UseQueryParamArrayResult<TValue extends string = string> =
  ReturnType<typeof useQueryParamArray<TValue>>;
