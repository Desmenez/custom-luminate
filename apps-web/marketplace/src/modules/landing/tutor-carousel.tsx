  ...
  return (
    <Carousel>
      {tutorCards.map((tutorCard) => (
        <NextLink key={tutorCard.id} href={`/browse/?tutorIds=${tutorCard.tutorId}`}>
          <NextImage
  ...
  return (
    <section className="flex flex-col items-center gap-y-6 lg:py-14 py-8">
      ...
      <div className="flex flex-row items-center justify-center w-full gap-x-4">
        {scrollable && (
          <button type="button" className="absolute left-0 lg:relative z-50" onClick={prev}>
            <FontAwesomeIcon
              icon={faChevronLeft}
        ...
        {scrollable && (
          <button type="button" className="absolute right-0 lg:relative z-50" onClick={next}>
            <FontAwesomeIcon
              icon={faChevronRight}
