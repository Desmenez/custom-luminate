...
} from "./sections/page-header-section";
import { ReviewSection } from "./sections/review-section";
import {
  TabSection,
  TabSectionProps,
  TabSectionRef,
} from "./sections/tab-section";
import {
  TutorProfileSection,
  TutorProfileSectionProps,
} from "./sections/tutor-profile-section";

export const CourseDetailPage = withSSRError<typeof getServerSideProps>(
  (props) => {
    ...
    const showImageDescriptionSection =
      course.type === LiveCourseType.ONSITE &&
      course.liveCourseImageDescription.length > 0;
    const showLessonSection = course.type !== LiveCourseType.ONSITE;
    const showExamSection =
      course.examTutorials.length > 0 ||
      course.exams.length > 0 ||
      course.mockExams.length > 0;
    const showSuggestedCoursesSection = course.suggestedNextCourses.length > 0;
    const showFundamentalCoursesSection =
      course.suggestedPrerequisiteCourses.length > 0;
        ...
        <CourseDetailLayout
          ...
          left={
            <>
              ...
              {showExamSection && (
                <ExamSection ref={examSectionRef} {...examSectionProps} />
              )}
...
export const getServerSideProps = withApi(async (context, api) => {
  ...
  const data = await extendFieldsItem(
    ...
    async () => {
      return {
        course: await api.liveCourse.getLiveCourseById({
          params: { id: courseId },
        }),
      };
    },
  ...
  const courseResult = {
    ...course,
    liveSessions: liveSessions.map((session, index) => ({
      index: index + 1,
      ...session,
    })),
  } as const;
